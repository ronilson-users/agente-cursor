...
`)}countActiveRules(){let e=this.getRules();return Object.values(e).filter(t=>t!==!1).length}reset(){this.toggleRules={preferTypescript:!1,preferCleanCode:!0,explainLikeTeacher:!1,simpleLanguage:!1,showAlternativeSolutions:!1,optimizePerformance:!1,autoDetectLanguage:!0},this.customRules={},this.save()}};var v=class{static async copyToClipboard(e){if(!e)return!1;try{if(navigator.clipboard&&navigator.clipboard.writeText)return await navigator.clipboard.writeText(e),!0}catch{}try{let t=document.createElement("textarea");t.value=e,t.style.position="fixed",t.style.left="-9999px",document.body.appendChild(t),t.select(),t.setSelectionRange(0,t.value.length);let r=document.execCommand("copy");return document.body.removeChild(t),!!r}catch(t){return console.error("Fallback copy failed:",t),!1}}static async loadHighlight(e){let t=o=>new Promise((a,n)=>{if(document.querySelector(`script[src="${o}"]`))return a();let i=document.createElement("script");i.src=o,i.onload=()=>a(),i.onerror=()=>n(new Error("Failed to load script "+o)),document.head.appendChild(i)}),r=o=>{if(document.querySelector(`link[href="${o}"]`))return;let a=document.createElement("link");a.rel="stylesheet",a.href=o,document.head.appendChild(a)};try{e||(e=""),r(`${e}assets/highlight-light.min.css`),await t(`${e}assets/highlight.min.js`);let o=window.hljs;return o&&typeof o.highlightAll=="function"&&(o.configure&&o.configure({ignoreUnescapedHTML:!0}),o.highlightAll()),!0}catch(o){return console.warn("N\xE3o foi poss\xEDvel carregar highlight.js:",o),!1}}};var w=class{cryptoKey=null;encoder=new TextEncoder;decoder=new TextDecoder;async encrypt(e){if(!e)return"";let t=await this.loadCryptoKey(),r=crypto.getRandomValues(new Uint8Array(12)),o=this.encoder.encode(e),a=await crypto.subtle.encrypt({name:"AES-GCM",iv:r},t,o),n=new Uint8Array(a),i=new Uint8Array(r.length+n.length);i.set(r),i.set(n,r.length);let s="";for(let d=0;d<i.length;d++)s+=String.fromCharCode(i[d]);return btoa(s)}async decrypt(e){if(!e)return"";try{let t=await this.loadCryptoKey(),r=atob(e),o=new Uint8Array(r.length);for(let s=0;s<r.length;s++)o[s]=r.charCodeAt(s);let a=o.slice(0,12),n=o.slice(12),i=await crypto.subtle.decrypt({name:"AES-GCM",iv:a},t,n);return this.decoder.decode(i)}catch(t){return console.error("Erro ao descriptografar:",t),""}}async loadCryptoKey(){if(this.cryptoKey)return this.cryptoKey;let e="agenteIA.secretKey",t=localStorage.getItem(e);if(!t){let a=this.randomBytes(32),n=String.fromCharCode(...a),i=btoa(n);localStorage.setItem(e,i),t=i}let r=atob(t),o=new Uint8Array(r.length);for(let a=0;a<r.length;a++)o[a]=r.charCodeAt(a);return this.cryptoKey=await crypto.subtle.importKey("raw",o,{name:"AES-GCM",length:256},!1,["encrypt","decrypt"]),this.cryptoKey}randomBytes(e){let t=new Uint8Array(e);return crypto.getRandomValues(t),t}};var l=class{static escapeHtml(e){let t=document.createElement("div");return t.textContent=e,t.innerHTML}static autoResizeTextarea(e){e.style.height="auto",e.style.height=Math.min(e.scrollHeight,320)+"px"}static createElement(e,t={},r=[]){let o=document.createElement(e);return Object.entries(t).forEach(([a,n])=>{o.setAttribute(a,n)}),r.forEach(a=>{typeof a=="string"?o.appendChild(document.createTextNode(a)):o.appendChild(a)}),o}static loadStylesheet(e){let t=this.createElement("link",{rel:"stylesheet",href:e});return document.head.appendChild(t),t}};var S=class{static render(e){return`
     ...
     <div class="chat-container">
                ...
                <div id="chat-messages" class="chat-messages scroll">
                    <div class="message system">
                        <p>
                            ${this.userProfile(e)}Como posso ajudar voc\xEA
                            hoje?
                        </p>
                    </div>
                </div>
    ...
    `}};var T=class{static render(e){return`
    
    <div class="config-container scroll">
      <div class="sections">
        <h3 class="config-title">Configura\xE7\xF5es de API</h3>
        ...
        <div class="config-section">
          ...
          <small class="text-muted">Este nome ser\xE1 usado nas sauda\xE7\xF5es
          </small>
        </div>
        ...
        <div class="config-section ">
          <label for="temperature" class="block ">Temperatura: <span id="temp-value">${e.temperature||.7}</span></label>
...
`)),P=l.escapeHtml(h.lang||"text"),z="code-"+Date.now()+"-"+Math.random().toString(36).slice(2,9);return`
    ...
    `,r.appendChild(o),r.scrollTo({top:r.scrollHeight,behavior:"smooth"})}setAnalyzeButtonState(e){let t=this.container.querySelector("#analyze-context");if(t)switch(e){case"loading":t.innerHTML="\u23F3 Analisando...",t.disabled=!0;break;case"error":t.innerHTML="\u274C Erro",t.disabled=!1,setTimeout(()=>{t.innerHTML="\u{1F50D} Analisar Contexto"},2e3);break;default:t.innerHTML="\u{1F50D} Analisar Contexto",t.disabled=!1}}showError(e){acode.require("toast")(e)}};var A=acode.require("sidebarApps"),I=acode.require("toast"),y=acode.require("settings"),H=class{contextAnalyzer;container=null;id=m.id;ruleManager;aiService=null;baseUrl="";cryptoUtils;$mainStyle=null;constructor(){this.ruleManager=new E,this.cryptoUtils=new w,this.initializeSettings()}initializeSettings(){y.value[m.id]||(y.value[m.id]={apiKey:"",provider:"openai",model:"gpt-4o-mini",temperature:.7,userName:"",rules:this.ruleManager.getRules()},y.update())}async init(){try{let e=`${this.baseUrl}assets/icon.png`;acode.addIcon("sidebar-icon",e),this.loadGlobalStyles(),await this.loadHighlightJS(),A.add("sidebar-icon","agente_IA","Agente IA",t=>{this.container=t,this.renderContainer()},()=>this.onAppSelected()),console.log("Plugin inicializado com sucesso")}catch(e){console.error("Erro ao inicializar plugin:",e)}}loadGlobalStyles(){this.$mainStyle=l.loadStylesheet(`${this.baseUrl}main.css`)}async loadHighlightJS(){try{typeof v.loadHighlight=="function"&&await v.loadHighlight(this.baseUrl)}catch(e){console.warn("N\xE3o foi poss\xEDvel carregar highlight.js:",e)}}renderContainer(){if(!this.container)return;let e=this.getSettings();this.container.innerHTML=`
    ...
    `,this.initializeUIComponents()}initializeUIComponents(){if(!this.container)return;new k(this.container).initialize(),new M(this.container,a=>this.sendMessage(a),()=>this.clearChat()).initialize(),new f(this.container,this.ruleManager,()=>this.refreshRulesUI()).initialize(),new C(this.container,()=>this.saveConfig(),()=>this.testAPI(),this.cryptoUtils).initialize(),this.contextAnalyzer=new L(this.container),this.contextAnalyzer.renderAnalyzeButton(),this.loadSavedConfig()}async sendMessage(e){u.addMessage(this.container,"user",e),u.showTypingIndicator(this.container);try{let t=this.getSettings();if(!t.apiKey)throw new Error("Configure uma API Key primeiro na aba Config");let r=await this.cryptoUtils.decrypt(t.apiKey);if(!r)throw new Error("API Key inv\xE1lida");this.aiService=new x({apiKey:r,provider:t.provider,model:t.model,temperature:t.temperature});let a=`
REGRAS DO SISTEMA:
...
Por favor, responda seguindo as regras acima.`.trim(),n=await this.aiService.sendMessage(a);u.hideTypingIndicator(this.container),u.addMessage(this.container,"assistant",n)}catch(t){u.hideTypingIndicator(this.container);let r=t?.message??String(t);u.addMessage(this.container,"system",`Erro: ${r}`),console.error("sendMessage error:",t)}}clearChat(){let e=this.container?.querySelector("#chat-messages");if(!e)return;e.innerHTML="";let t=this.getSettings();u.addMessage(this.container,"system",`${t.userName?`Ol\xE1, ${t.userName}! `:"Ol\xE1! "}Como posso ajudar voc\xEA hoje?`)}refreshRulesUI(){let e=this.container.querySelector(".rules-content");e&&(e.innerHTML=b.render(this.ruleManager),new f(this.container,this.ruleManager,()=>this.refreshRulesUI()).initialize())}async saveConfig(){console.log("\u{1F504} saveConfig chamado");let e=this.container?.querySelector("#api-key"),t=this.container?.querySelector("#api-provider"),r=this.container?.querySelector("#api-model"),o=this.container?.querySelector("#temperature"),a=this.container?.querySelector("#user-name");if(console.log("\u{1F511} API Key input value:",e?.value||"vazio"),console.log("\u{1F3F7}\uFE0F Provider:",t?.value),console.log("\u{1F916} Model:",r?.value),console.log("\u{1F321}\uFE0F Temperature:",o?.value),!e||!t||!r||!o){console.error("\u274C Elementos DOM n\xE3o encontrados"),acode.require("toast")("Campos de configura\xE7\xE3o incompletos");return}let n=this.getSettings();console.log("\u{1F4C1} Settings antes:",n);let i=e.value.trim();if(i!==""){console.log("\u{1F510} Criptografando nova chave...");try{n.apiKey=await this.cryptoUtils.encrypt(i),console.log("\u2705 Chave criptografada e salva")}catch(s){console.error("\u274C Erro ao criptografar:",s)}}else console.log("\u26A0\uFE0F API Key vazia, mantendo anterior");n.provider=t.value,n.model=r.value,n.temperature=parseFloat(o.value),n.userName=a?.value||n.userName,console.log("\u{1F4C1} Settings depois:",n);try{y.update(),console.log("\u{1F4BE} Configura\xE7\xF5es salvas no appSettings"),acode.require("toast")("Configura\xE7\xF5es salvas!");let d=this.container?.querySelector("#model-indicator");d&&(d.textContent=`Modelo: ${n.model}`,console.log("\u{1F504} Model indicator atualizado"))}catch(s){console.error("\u274C Erro ao salvar configura\xE7\xF5es:",s)}}testAPI(){I("Testando conex\xE3o com API..."),setTimeout(()=>{I("Conex\xE3o com API bem-sucedida!")},1500)}async loadSavedConfig(){if(!this.container)return;let e=this.getSettings();if(!e)return;let t=this.container.querySelector("#api-key"),r=this.container.querySelector("#api-provider"),o=this.container.querySelector("#api-model"),a=this.container.querySelector("#temperature"),n=this.container.querySelector("#temp-value"),i=this.container.querySelector("#user-name");if(r&&e.provider&&(r.value=e.provider),o&&e.model){o.value=e.model;let s=this.container.querySelector("#model-indicator");s&&(s.textContent=`Modelo: ${e.model}`)}if(a&&e.temperature!==void 0&&(a.value=e.temperature.toString(),n&&(n.textContent=e.temperature.toString())),i&&e.userName&&(i.value=e.userName),t)if(e.apiKey)try{let s=await this.cryptoUtils.decrypt(e.apiKey);t.value=s||""}catch(s){console.error("Erro ao carregar apiKey:",s),t.value=""}else t.value=""}getSettings(){return y.value[m.id]||{}}onAppSelected(){I("Agente IA ativado")}async destroy(){A&&(A.remove("agente_IA"),A.remove("sidebar-icon")),this.$mainStyle&&this.$mainStyle.remove()}};if(window.acode){let c=new H;acode.setPluginInit(m.id,async(e,t,{cacheFileUrl:r,cacheFile:o})=>{e.endsWith("/")||(e+="/"),c.baseUrl=e,await c.init()}),acode.setPluginUnmount(m.id,()=>{c.destroy()})}})();
